...
if (empty($_POST["name"])) {
  ...
  $nameempty = "Admin Name required";
}
if (empty($_POST["phoneno"])) {
  //storing error message in $fnameempty
  $phoneempty = "Admin Phone no required";
}
    $row1_c1 = test_input($_POST['name']);
    $row1_c3 = test_input($_POST['phoneno']);
   
    if (strlen($row1_c1) && strlen($row1_c3) >0){
    $sql = "INSERT INTO `users`  (`username`, `phoneno`) VALUES 
    ('$row1_c1', '$row1_c3')
    ";
    //sql query
...
<?php require 'partials/_navadmin.php' ?>
 ...
 <h1 class="text-white text-center">  Add Admin </h1>
 </div>
 <form method="post">
 <br>
